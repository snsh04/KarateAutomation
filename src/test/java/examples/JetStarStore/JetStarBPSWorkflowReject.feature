...
And request read('JetStarBPSRequest.xml')
And soap action ''
Then status 200
* match /Envelope/Body/simpleQueryResponse/taskSimpleQueryResultSet/row[1]/presentationSubject contains 'Auto_Reject'
...
* def xml = <sch:SubscriptionApprovalResponse xmlns:sch="http://workflow.subscription.apimgt.carbon.wso2.org"><sch:status>REJECTED</sch:status><sch:workflowExternalRef>#(ref)</sch:workflowExternalRef><sch:description></sch:description></sch:SubscriptionApprovalResponse>
...
* call read('JetStarBPSWorkflowCompletion.feature')
