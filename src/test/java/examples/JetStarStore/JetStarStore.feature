...
@Store @parallel=false @All @JetstarStore
Scenario Outline: Subscribe to an API
#Register and API
Given url Admin
* json myReq = read('Swagger-JetStarSubscriber.json')
* def name = '1-' + now()
...
* eval if (response.status == 'ON_HOLD') karate.call('JetStarBPSWorkflow.feature')
...
* def responseFromAuthenticatedExternalWSO2Gateway = call read('classpath:examples/Services/InvokeAuthenticatedProdJetStarExternalWSO2Gateway.feature') {'domain': '#(domain)' , 'basepath': '#(basepath)' , 'path': '#(path)' , 'externalGatewayResponse': '200' , 'method': '#(requestMethod)' , 'accessTokenforProd': '#(accessTokenforInvokation)' }
...
|'v66'  |'Auto_ApprovalJ'        |'partner'  |'countryJetS00'|'regtest'                    |'admin' |'Gold'              |'ON_HOLD'  |'get'|'quote'|
|'v397' |'REGTEST_SubscribeJet1' |'aircraft' |'countryJetS1'|'regtest_tech@random.com' |'admin' |'Gold_Auto-Approved'|'UNBLOCKED'|'get'|'quote'|
|'v88'  |'REGTEST_SubscribeJet2' |'flight'   |'countryJetS2'|'regtest_buss@random.com' |'admin' |'Gold'              |'UNBLOCKED'|'get'|'quote'|
|'v799' |'REGTEST_SubscribeJet3' |'car'      |'countryJetS3'|'regtest_tech@random.com' |'admin' |'Gold'              |'UNBLOCKED'|'get'|'quote'|
|'v555' |'REGTEST_SubscribeJet4' |'customer' |'countryJetS4'|'regtest_buss@random.com' |'admin' |'Gold_Auto-Approved'|'UNBLOCKED'|'get'|'quote'|
